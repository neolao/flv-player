...
class PlayerBasic
{
    ...
    /**
     * After seeking, where the video started
     */
        ...
        this._ns.onStatus = function(info:Object){
            switch(info.code){
                ...
                case 'NetConnection.Connect.Success':
                    ...
                    myStream.onResult = function(streamLength:Number) {
                        this.parent.parent._videoDuration = (info.duration < 0)?0:info.duration; // + this.parent.parent_offset;
                    };
    ...
    public function setPosition(pPosition:Number)
    {
        ...
        if (this._isPhpStream) {
            var objLoading:Object = this.getLoading();
            // If the point we're seeking to is already loaded, don't use phpStream.
            // ES: Well, I'm not sure how to do that, because we may have seeked already,
            //  and I don't know if the bytesLoaded amount refers to the total for the stream
            //  or the total remaining from where we seeked to. I'll try and do some
            //  heuristics, so it'll be better, but not perfect.
            if (this._offset == 0 && objLoading.percent == 100) {
                // Fully downloaded. Just seek.
            ...
            if (pPosition == 0) {
                newPosition = 0;
                newTime = 0;
            } else if (pPosition <= this._times[0]) {
                newPosition = this._positions[0];
                newTime = this._times[0];
            ...
            // ES: Heuristics here.
            if (newPosition >= this._offset && (objLoading.percent == 100 || this._offset + objLoading.loaded >= newPosition)) {
                // I have the data loaded already. Just seek.
                this._ns.seek(pPosition);
                return;
            }
    ...
    public function getPosition():Number
    {
        ...
        pos = this._ns.time; // + this._offset;

        if (pos > this._videoDuration) {
